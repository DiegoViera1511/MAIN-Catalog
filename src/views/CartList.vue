...
const cartTotalCount = computed(() => store.getCartItemsCount());
const cartTotalPrice = computed(() => store.getCartTotal());
...
function onClickMinusButton(product: CartProduct) {
...
<template>
  ...
  >
    ...
    <div
      class="flex flex-row gap-2 w-full items-center justify-start text-xl"
      v-if="cartTotalCount > 0"
    >
      <p class="text-gray-500">
        {{ cartTotalCount }}
        {{ cartTotalCount === 1 ? "producto" : "productos" }}
      </p>
    ...
    <div class="flex flex-col gap-3 w-full">
      <div class="flex w-full items-center justify-center">
        <span
          ...
          class="font-medium text-2xl text-gray-500"
          >No hay productos en tu lista de compras.</span
        >
      </div>
      <div
      ...
      >
        <div class="flex flex-row w-full items-center justify-start gap-2">
          <img
            :src="product.url"
            alt="product image"
            class="w-[150px] bg-gray-200 h-[150px] object-cover rounded-md"
          />
          <div
            class="flex flex-col flex-1 items-start justify-start font-medium text-lg w-[150px] h-[150px]"
          >
            <span>{{ product.title }}</span>
            <span class="text-neutral-500" v-if="product.selectedSize"
              >Talla: {{ product.selectedSize }}</span
            >
            <span class="text-neutral-500"
              >Color: {{ SpanishColors[product.selectedColor as Colors] }}</span
            >
            <div>
        ...
        >
          ...
          <button
            class="flex items-center hover:bg-gray-100 dark:hover:bg-neutral-700 justify-center rounded-full p-2"
            @click="() => onClickMinusButton(product)"
          >
